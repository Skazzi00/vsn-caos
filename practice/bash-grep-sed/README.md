...
they all use the shell
bash rule bash is the best
```

```
# только одна строка, которая содержит слово "cat"
> grep cat test.txt
the cat sat on the mat

# строки, которые содержат подстроку " the "
> grep ' the ' test.txt
the cat sat on the mat
the dog ran in the park
...
```
# найти все строки, в которых слово из латинских букв [a-zA-Z]
...
```
# заменить слова "cat" и "dog" на "pet"
> sed -E 's/(cat|dog) /pet /' test.txt
the pet sat on the mat
the pet ran in the park
code runs on the host
they all use the shell
bash rule bash is the best
```
...
> sed -E 's/([a-zA-Z]+) ([a-zA-Z]+) (.*)/\2 \1 \3/' test.txt
cat the sat on the mat
dog the ran in the park
runs code on the host
all they use the shell
rule bash bash is the best

# то же самое, но только для строк, начинающихся с буквы "t"
> sed -E '/^[t].*/s/([a-zA-Z]+) ([a-zA-Z]+) (.*)/\2 \1 \3/' test.txt
cat the sat on the mat
dog the ran in the park
code runs on the host
they all use the shell
